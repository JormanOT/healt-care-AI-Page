  ...
  return (
    <>
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-4 md:px-6 py-2 md:py-3 bg-white/80 backdrop-blur-md">
        ...
        >
          <span
            className={`absolute h-0.5 w-6 bg-black rounded-full transition-all duration-300 ease-[cubic-bezier(0.76,0,0.24,1)] ${mobileOpen ? 'rotate-45 translate-y-0' : '-translate-y-2'
              }`}
          />
          <span
            className={`absolute h-0.5 w-6 bg-black rounded-full transition-all duration-300 ease-[cubic-bezier(0.76,0,0.24,1)] ${mobileOpen ? 'opacity-0 scale-x-0' : 'opacity-100 scale-x-100'
              }`}
          />
          <span
            className={`absolute h-0.5 w-6 bg-black rounded-full transition-all duration-300 ease-[cubic-bezier(0.76,0,0.24,1)] ${mobileOpen ? '-rotate-45 translate-y-0' : 'translate-y-2'
      ...
      {/* Menu overlay */}
      <div
        className={`fixed inset-0 z-40 transition-none ${mobileOpen ? 'pointer-events-auto' : 'pointer-events-none'
          }`}
      >
        {/* Backdrop */}
        <div
        ...
        {/* Panel */}
        <div
          className={`absolute top-0 right-0 h-full w-[85%] max-w-sm md:w-[30%] md:max-w-none bg-white shadow-2xl transition-transform duration-500 ease-[cubic-bezier(0.76,0,0.24,1)] ${mobileOpen ? 'translate-x-0' : 'translate-x-full'
            }`}
        >
          ...
          <div
            className={`absolute bottom-0 left-0 right-0 px-8 py-8 border-t border-neutral-200 transition-all duration-500 ease-[cubic-bezier(0.76,0,0.24,1)] ${mobileOpen ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'
              }`}
